...
void* KA_czlonek3() {
    while (!terminate_threads) {
        printf("Liczba studentow do przepytania w CZLONKU3: %d\n",Liczba_studentow_do_zegzaminowania);
        for (int KACZLON3 = 0; KACZLON3 < 2 * Liczba_studentow_do_zegzaminowania; KACZLON3++) {
            ...
            if (msg.zdaje_czy_pytania == 0) {
                if (msg.czy_zdane != 1) {
                    ...
                    KACZLON3++; // bo generalnie kazdy student wysyla dwa komunikaty, chyba ze przepisuje (to jest tylko zeby iterowanie bylo poprawne)
                }
...
void* KA_czlonek2(){
    while (!terminate_threads) {
        printf("Liczba studentow do przepytania w CZLONKU2: %d\n",Liczba_studentow_do_zegzaminowania);

        for (int KACZLON2 = 0; KACZLON2 < 2*Liczba_studentow_do_zegzaminowania; KACZLON2++) {
            //czeka na informacje od Przewodniczacego na temat nowego studenta
            ...
            if(msg.zdaje_czy_pytania== 0){
                if(msg.czy_zdane != 1){ //-jak nie zdane to w tym przypadku chce pytania
                    ...
                    KACZLON2++; // bo generalnie kazdy student wysyla dwa komunikaty, chyba ze przepisuje (to jest tylko zeby iterowanie bylo poprawne)
                }
...
int main(){
    ...
    printf("\n\n\nLiczba osob do zegzaminowania: %d\n\n\n",Liczba_studentow_do_zegzaminowania);
    ...
    switch(fork())
    {
        ...
        */
        ...
        printf("Liczba studentow do przepytania w przewodniczacym: %d\n",Liczba_studentow_do_zegzaminowania);
        for (int PAE = 0; PAE < 2*Liczba_studentow_do_zegzaminowania; PAE++) {
         
            //czeka na komunikat od studenta i sprawdza, czy chce pytania czy już zdawać
